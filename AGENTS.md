...
`/commit`, `/pr`, `/pr release`, `/issue`, `/review-reply`, `/code-review`, `/handoff`, `/explain-diff` 여덟 가지 slash command를 제공한다.
...
- main push 후 Codex 동기화 (**`npx skills update` 사용 금지 — 직접 복사**):
  ...
  for s in commit pr issue review-reply code-review handoff explain-diff; do
...
npx skills remove -g commit pr issue review-reply code-review handoff explain-diff -y
