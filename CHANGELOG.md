...
**Fixed**
- `/explain-diff` 구조 트리가 계층 없이 평평하게 나오던 문제 — 실제 산출물 피드백 반영. 템플릿의 트리 행 예시가 `├─ {{PATH}}` 한 줄뿐이고 SKILL.md에 중첩 규칙이 없어, 건드린 파일마다 전체 경로(`graph/workflows/file_indexing.py`, `graph/models/llm/utils.py` …)를 같은 깊이에 나열해 트리라기보다 파일 목록이 되던 문제. 템플릿을 중첩 예시(디렉토리 행 + `│  ├─`/`└─` 가이드 prefix + 삭제 행)로 교체하고 `.guide`(gray300, 이름보다 연하게)·`.dir`(gray700 굵게) 스타일을 추가해 들여쓰기가 먼저 읽히게 했다. SKILL.md에 트리 구성 규칙 명시: 행은 자기 이름만 표시하고 부모 아래 중첩, 가이드 prefix 계산법(형제가 남은 조상은 `│  `, 닫힌 조상은 공백, 마지막 형제는 `└─`), 단일 자식 디렉토리 체인·같은 방식으로 바뀐 형제 파일은 한 행으로 접기(`config.{prod,beta}.yaml`), 디렉토리 행 칩은 디렉토리 전체가 신설·삭제된 경우에만, 디렉토리 우선 알파벳 정렬, 4단계 초과 깊이는 접기 신호. Chrome headless 렌더링으로 검증 (explain-diff 1.1.3 → 1.1.4)

**New**
- `/explain-diff` 긴 트리 자동 접기 — 계층 트리로 바뀌면서 디렉토리 행이 추가돼 대형 PR에서는 개요 파트가 트리 카드에 잠식되는 문제. 템플릿 JS가 `.tr-row`를 세어 20행을 넘으면 `.capped`를 붙이고 페이드와 `나머지 n개 행 펼치기` 버튼을 자동으로 단다 (20행 이하 문서는 아무 변화 없음, 스킬이 마크업을 손댈 필요 없음). 펼치기·접기는 `max-height`를 실측 px 값 사이에서 전환해 부드럽게 움직이고, 펼친 뒤에는 `max-height`를 풀어 이후 레이아웃 변화에도 잘리지 않게 한다. 접을 때 트리 머리가 뷰포트 위로 벗어나면 기존 섹션 이동 함수로 되돌려 사용자가 위치를 잃지 않게 하고, `prefers-reduced-motion`이면 즉시 전환. 세로 제한 없이 두는 안(A)과 비교 페이지로 검토한 뒤 채택. 삭제 행의 취소선이 가이드 문자까지 그어지던 문제도 `.guide`를 `inline-block`으로 바꿔 함께 수정
- `/explain-diff` disclosure(`details.fold`) 스타일 신설 — 실제 산출물에서 트리를 브라우저 기본 `<details>`로 감싸 `▶ 전체 변경 파일 47개 보기` 같은 summary가 템플릿 폰트 규칙을 타지 않고 따로 놀던 문제. 템플릿에 없는 컴포넌트를 모델이 즉석에서 만든 경우로, (1) SKILL.md에 "트리는 항상 로드 시 보이며 `<details>`나 요약 카드로 대체하지 않는다, 헤더 문구 `디렉토리 변화` 유지"를 명시하고 (2) 다른 곳에서 접기가 필요할 때를 위해 본문 타이포그래피를 상속하는 `details.fold`(커스텀 chevron, hover·focus 상태)를 팔레트에 추가
...
**Changed**
- `/handoff` 지시문 전제 미세 조정 — 실제 사용 피드백 반영. (1) "사용자가 후속 작업에 맞는 작업 디렉토리에서 새 세션을 연다"는 가정을 One Principle에 명시해, 저장소 경로·실행 위치 안내가 자연스럽게 노이즈로 분류되도록 함 (금지 규칙 추가가 아닌 전제 명시). (2) 예시 해설의 "'상황을 파악하라'-style preamble 금지" 문구를 "지시 없는 요약 금지"로 한정 — 다음 에이전트가 작업 전 저장소를 탐색할지는 에이전트 스스로 정하는 것이므로, handoff가 탐색을 시키지도 막지도 않는다는 중립 전제로 완화

## v1.18.0 (2026-08-10)

**New**
- `/visual-doc` `.wide` 컴포넌트 신설 (오른쪽 단방향 확장) — 실제 산출물 피드백 반영. 셀 텍스트가 억지로 두 줄로 꺾이는 표를 본문 컬럼(800px)보다 넓게 쓰고 싶다는 요구에서 출발. 컬럼 전체 확대(940px)는 산문 행 길이가 길어지고, `.bleed` 방식의 양쪽 대칭 확장은 챕터 제목·산문과 왼쪽 축이 어긋나 어색해지는 것을 실제 산출물 스크린샷으로 확인한 뒤, 왼쪽 축은 본문 컬럼과 공유하고 오른쪽(TOC rail 직전)으로만 확장하는 안으로 확정. 소제목이 딸린 블록은 소제목을 `.wide` 래퍼 안에 함께 넣어 같은 축에 정렬하고, 표의 첫 열·마지막 열(라벨·링크)은 `white-space:nowrap`으로 한 줄 고정. 산문·콜아웃·TL;DR은 기본 폭 유지. SKILL.md에 "Wide blocks extend right only" 규칙과 적용 기준을 추가하고 팔레트에 데모 표 수록
- `/visual-doc` 본문 링크 기본 스타일 팔레트 승격 — 산출물마다 즉석 정의하던 `a` 스타일(`--chip-ink` 텍스트 + 반투명 밑줄 border, hover 시 진해짐)을 팔레트 기본으로 수록. 출처·이슈·PR 링크가 거의 항상 들어가는 스킬 특성상 문서 간 링크 스타일 일관성 확보. 링크형 컴포넌트(`.toc-rail a`, `.pg-btn`)에는 `border-bottom:none` 리셋을 함께 적용해 밑줄 번짐 방지

**Changed**
- `/visual-doc` 챕터 제목(`.group-label`) 확대 — 실제 산출물 피드백 반영. 챕터 제목(`.g-name` 13.5px)이 카드 소제목 `h2`(19px)·본문(15px)보다 작아 위계가 역전되고 챕터 구분감이 약하던 문제. `.g-name` 13.5px/700 → 19.5px/800 (letter-spacing -0.025em, `word-break:keep-all`), `.g-part` 11px/600 → 13px/700, 상단 여백 52px → 60px로 조정하고, 커진 제목에 맞춰 구분선을 `--line` → `--gray200`으로 한 단계 진하게 변경

## v1.17.2 (2026-08-01)

**Fixed**
- `/visual-doc` Q&A 블록에 카드 배경 추가 — 실제 산출물 피드백 반영. `.qa`가 테두리만 있는 아웃라인 블록이라 회색 페이지 배경 위에서 흰 카드 블록들 사이에 있으면 미완성처럼 보이던 문제. `background:var(--card)`를 주어 다른 블록과 같은 면 위계로 통일. 배경 없음 / 카드 배경 / 블루 틴트(`--blue-soft`) 세 안을 나란히 비교한 뒤 사용자가 카드 배경을 선택했으며, 블루 틴트는 TL;DR·콜아웃이 쓰는 "강조 면" 의미와 겹쳐 제외

## v1.17.1 (2026-08-01)

**Fixed**
- `/visual-doc` 최상위 블록 간 간격 규칙 추가 및 기본 간격 20px 확정 — 실제 산출물 피드백 반영. 간격 규칙(`margin-top`)이 `.stack` 래퍼 내부에만 정의되어 있어, 섹션에 바로 놓인 card·codeblock·callout 등 최상위 블록끼리는 간격 0으로 붙어 보이던 문제. `.wrap` 직속 블록 12종(card, codeblock, callout, qa, pillars, proscons, stats, statchips, stack, tldr, takeaway, bleed)에 같은 간격을 적용하고, `.demo-label` 바로 다음 블록은 라벨과 밀착 유지. 기본 간격은 실제 보고서에서 24/20/16px을 라이브 토글로 비교한 뒤 사용자가 20px로 확정(v1.16.0의 24px에서 조정), 연속 콜아웃은 12px. 배경과 근거: docs/decisions/2026-07-visual-doc.md의 "간격" 항목

**New**
- `/visual-doc` 다이어그램 컴포넌트 신설 (full-bleed + 라이트박스) — 실제 산출물 피드백 반영. mermaid는 SVG를 컨테이너 폭에 맞춰 통째로 축소하므로 요소가 많은 다이어그램(예: participant 5개 시퀀스)이 800px 본문 컬럼에서 읽기 어렵게 작아지던 문제. (1) `.bleed`: 다이어그램 카드만 본문 컬럼을 깨고 뷰포트 폭(최대 1200px)으로 확장, 1280px 이상에서는 고정 TOC rail과 겹치지 않게 좌우 240px씩 제외. (2) `.zoomable` + `.lightbox`: 클릭하면 전체 화면 오버레이에서 휠 줌(스크롤 양 비례 지수 배율, 트랙패드 대응) + 드래그 팬 + 더블클릭 리셋 + Esc/바깥 클릭 닫기. 드래그 종료 시 발생하는 click으로 창이 닫히던 오작동은 이동량 추적으로 차단. 배치 4종(현재/full-bleed/라이트박스/가로 스크롤)과 조합안을 실제 다이어그램으로 비교한 뒤 사용자가 선택한 사양이며, SKILL.md에 "Diagrams break out and zoom" 컴포넌트 규칙을 추가

**Fixed**
- `/visual-doc` 산출물 레이아웃·시인성 개선 — 실제 산출물 피드백 반영. (1) 본문+TOC를 1080px 그리드로 묶어 통째로 중앙 정렬하던 `.shell`을 본문 단일 컬럼(800px) 중앙 정렬로 바꾸고, TOC rail은 뷰포트 오른쪽 끝에 고정(`position:fixed`, 1280px 미만은 숨김, 뷰포트보다 길면 자체 스크롤) — 본문이 왼쪽으로 밀리고 TOC가 본문에 붙어 보이던 문제 해소. (2) `.prose`·`.lede`의 `max-width:64ch` 제거 — 한글 텍스트에서 `ch` 단위가 약 480px로 계산되어 카드 폭의 60%만 채우던 문제. (3) 본문성 텍스트 13곳의 `word-break:keep-all` 제거 — 긴 어절이 통째로 다음 줄로 내려가 오른쪽이 비어 보이던 문제. 줄 채움을 우선하고 음절 단위 개행을 허용하는 트레이드오프이며, 제목·라벨류 5곳은 공유 규칙(제목 keep-all)대로 유지. (4) 인라인 코드 칩을 브랜드 블루 틴트로 교체 — 배경 `color-mix(in srgb, var(--blue) 9%, transparent)` + 전용 글자색 토큰 `--chip-ink`(라이트 #1659C9 / 다크 #7FB4FF) + `font-weight:500`, 테두리 없음. 기존 `--chipbg` 고정 배경이 흰 카드 위에서 거의 구분되지 않던 시인성 문제("Color must survive its background" 규칙을 팔레트 자신이 위반)를 해소하며, 반투명 틴트라 흰 카드·회색 패널·콜아웃 어디에나 자연 적층된다. `--blue-dark`는 다크 표면·`--blue-soft` 콜아웃 위에서 AA 대비 미달이라 칩 전용 토큰으로 분리(최악 표면 기준 라이트 5.1:1, 다크 6.3:1). 시안 5종(테두리 회색 칩, 잉크 틴트, 블루 틴트, 배경 없는 미니멀, 포인트 컬러 텍스트)을 실제 산출물 배경 3종에서 비교한 뒤 사용자가 선택한 값으로, TDS Badge의 weak+blue 조합과 같은 문법이다. (5) SKILL.md에 "Fill the column" 컴포넌트 규칙을 신설하고 공유 규칙 정본(docs/decisions/2026-07-visual-doc.md)에 visual-doc의 본문 keep-all 제외를 의도적 분기로 기록해 회귀 방지
- `/pr` explain-diff 옵트인 제안 문구 개선: "(기본 No)"는 모델 내부 규칙이 유저 문구로 새어 나온 것이라 제거하고 "원하시면 /explain-diff로 이 PR의 이해용 설명 문서를 만들어 드릴까요?"로 변경. 기본 No·재질문 금지 동작은 영어 지시문이 그대로 보장하며, /explain-diff 스킬명은 사용자가 스킬을 알게 되는 통로이므로 문장 안에 유지

## v1.16.0 (2026-07-27)

**Changed**
- `/visual-doc` 콜아웃·stat 컴포넌트 개편 — 실제 산출물 피드백 반영. (1) 콜아웃에 라벨 행 도입: 팔레트 제공 SVG 아이콘 + 범주 이름(참고/확인됨/주의/함정)을 본문 위에 명시하고, 텍스트 글리프(`i`, `×`) 아이콘 금지 (폰트에 따라 막대·점으로 렌더링되어 범주가 안 읽히던 문제). (2) 같은 범주 콜아웃 3개 이상 연속이면 `.group` 하나로 합침 (라벨 한 번 + 옅은 구분선, 경고 블록 반복 피로 제거). (3) 콜아웃 항목을 제목 줄 + 본문으로 계층화. 본문이 한 줄에 들어가면 흐르는 한 줄 그대로, 한 줄을 넘길 때만 문장마다 `.sent`로 감싸 문장 경계에서 줄바꿈 (한 줄 길이 카피 규칙 병행). (4) 블록 간격 14px → 24px, 연속 콜아웃 16px. (5) stat 카드를 라벨 선행(라벨 → 숫자 → 부연)으로 뒤집고 억지 단위 명사("갈래")·번역투 라벨 금지, 항목 최대 4개. (6) 지표 5개 이상용 `.statchips`(알약형 칩 나열) 컴포넌트 신설. 배경과 근거: docs/decisions/2026-07-visual-doc.md의 "6. 콜아웃·stat 표현 규칙"
- `/visual-doc` 코드 리뷰 반영: 콜아웃 라벨 텍스트를 `--ink`로 고정하고 변형 색은 아이콘에만 적용 (라이트 모드 WCAG AA 미달 해소), `.statchips` 숫자 색을 `--ink`로 전환 (다크 모드 AA 미달 해소), "제목 줄 + 본문" 규칙을 `.group` 항목 한정으로 명확화 (단일 콜아웃은 라벨 행이 제목 역할), `.d` clamp 발동 시 잘린 내용을 본문에 재기술하는 규칙 추가. 관련 PR: #61
...
**Changed**
- `/explain-diff` 산출물 제목을 명사형/개조식으로 전환 — hero 제목을 문장형(`~를 ~로 바꿨어요`)으로 강제하던 규칙을 없애고, 변경 주제를 가리키는 명사구(~20자)로 한 줄에 담게 했다. what/why는 lede가 지므로 제목이 문장을 짊어질 필요가 없다. 실제 산출물에서 `모델명에 upstage/ 접두가 붙어 채팅이 막혔어요` 같은 긴 문장형 제목이 좁은 폭에서 두 줄로 짤리던 문제 반영
- `/explain-diff`, `/micro-world` 템플릿에 한글 줄바꿈 `word-break: keep-all` 적용 — 기본값은 한글에서 어절을 무시하고 글자 단위로 끊어 "접두"가 "접/두"로 갈라졌다. 서사 텍스트(제목, lede, prose, 핵심 정리, 퀴즈, 리스크, 목차 설명)를 어절 단위로만 개행하고, 그래도 넘치는 긴 라틴 토큰만 `overflow-wrap: anywhere`로 쪼갠다. 코드/트리 블록의 `white-space`(pre·nowrap)와 micro-world 터미널의 `break-all`은 그대로 유지
- `/explain-diff` 산출물에 자연스러운 한국어 규칙 신설 — 대시(—, –) 전면 금지(콜론·괄호·쉼표로 대체), 억지 한자어·번역투 금지(`접두` → `앞에 붙는 upstage/` 또는 `prefix`, `회귀 커밋` → `되돌린 커밋`), 코드·식별자 용어는 억지 번역 없이 원문 유지. 번역투와 조어가 AI 작성 티를 남긴다는 피드백 반영
...
**Changed**
- `/explain-diff` 섹션 이동을 거리 비례 JS 애니메이션으로 교체 — native `scroll-behavior: smooth`는 거리와 거의 무관하게 duration이 고정이라 긴 문서에서 중간 구간 속도가 폭주했고, 그렇다고 즉시 이동으로 두면 "휙" 하는 단절감이 생겼다. `clamp(12 * sqrt(거리), 350ms, 900ms)` + `easeInOutCubic`으로 직접 움직인다 (실측: 가까운 이동 약 440ms, 최장 이동 약 880ms). 해시는 이동이 끝난 뒤 `history.replaceState`로 기록해 브라우저의 추가 점프가 애니메이션을 덮어쓰지 않게 하고, 착지 하이라이트는 `.card.landed` 클래스가 담당한다 (`:target`은 해시 URL 직접 진입용으로 유지). 섹션 착지 여백은 `scroll-margin-top` 16px에서 24px로 조정
- `/explain-diff` 산출물 문장 규칙 신설 — 한 문장 한 생각, 한 문단 3~4문장, lede 2~3문장 분리, 문단당 볼드 1곳. 생성 문서가 인과를 한 문장에 몰아넣어 눈에 안 들어온다는 피드백 반영
...
**Fixed**
- `/micro-world` 터미널 대기 상태에 프롬프트 표시 — 커서만 깜빡이던 것을 실제 셸처럼 `❯ ` 프롬프트 라인 안에서 깜빡이도록 수정. 명령 실행 시 대기 프롬프트 자리에 명령이 찍히는 흐름으로 자연스러워지고, 스텝 되감기 시 빈 프롬프트 라인이 중복 생성되지 않도록 복원 로직 보강 (template.html 정본 반영)

## v1.11.0 (2026-07-18)

**New**
- `/micro-world` 스킬 추가 — 특정 동작(상태 머신, 알고리즘, 데이터 변환, 프로토콜 흐름)을 직접 조작하며 체감하는 일회성 인터랙티브 HTML 시뮬레이션 생성. Papert의 micro-world 개념을 도입한 explain-diff의 형제 스킬로, 시각 언어(디자인 토큰)만 공유하고 구조는 동작별 bespoke. 적용성 게이트(시뮬 가치가 없는 변경은 정직하게 거절하고 /explain-diff 권장), fidelity 규칙(실제 코드 로직을 모델링하고 단순화 지점은 페이지에 명시), 인터랙션 원칙(입력이 결과를 바꿀 때만) 포함. 커맨드 센터 문법 확정: 가이드 단일 사이클(미션 바 + 권장 액션 하나만 활성화 + 인라인 재시작), 재생 컨트롤(스냅샷 기반 ‹ › 스텝 이동, → 키는 앞 히스토리가 없으면 권장 액션을 직접 실행, ← → R 키보드), 사실/해설 분리(환경엔 실제 출력만, 의미는 해설 스트립), 세계 상태 통합 카드, embodiment 소재별 선택(터미널/타임라인 스크럽/드래그 앤 드랍 구조 시각화/노브). template.html에 프레임과 범용 재생 런타임 JS 동봉. 산출물은 저장소 루트에 micro-world-<slug>.html로 저장하고 절대경로만 안내
