...
**Fixed**
- `/visual-doc` 산출물 레이아웃·시인성 개선 — 실제 산출물 피드백 반영. (1) 본문+TOC를 1080px 그리드로 묶어 통째로 중앙 정렬하던 `.shell`을 본문 단일 컬럼(800px) 중앙 정렬로 바꾸고, TOC rail은 뷰포트 오른쪽 끝에 고정(`position:fixed`, 1280px 미만은 숨김) — 본문이 왼쪽으로 밀리고 TOC가 본문에 붙어 보이던 문제 해소. (2) `.prose`·`.lede`의 `max-width:64ch` 제거 — 한글 텍스트에서 `ch` 단위가 약 480px로 계산되어 카드 폭의 60%만 채우던 문제. (3) 본문성 텍스트 11곳의 `word-break:keep-all` 제거 — 긴 어절이 통째로 다음 줄로 내려가 오른쪽이 비어 보이던 문제. 줄 채움을 우선하고 음절 단위 개행을 허용하는 트레이드오프이며, 제목·라벨류 5곳은 공유 규칙(제목 keep-all)대로 유지. (4) 인라인 코드 칩을 `--gray50` 배경 + 1px `--gray200` 테두리 + `--gray700` 글자색으로 강화 — 기존 `--chipbg` 배경이 흰 카드 위에서 거의 구분되지 않던 시인성 문제로, SKILL.md의 "Color must survive its background" 규칙을 팔레트 자신이 위반하던 것을 해소. 테두리가 회색 패널 위에서도 경계를 잡아주고, 옅은 배경이 흰 카드에서 과하게 진해 보이지 않게 한다. SKILL.md에 "Fill the column" 컴포넌트 규칙을 신설해 회귀 방지
