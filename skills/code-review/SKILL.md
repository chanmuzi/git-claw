...
| **`out-of-scope`** | Real, but not this change's doing — it predates the branch, or fixing it means restructuring something the change does not touch. | One line in the summary, under `밖으로 미룸`. **Never an inline comment.** |
...
- **`-a` / `--all`**: skip this step. Every Confirmed finding is reported at full length, `drop` included. The `기각` line is omitted (nothing was cut).
- **`--full-scan`**: promotes `out-of-scope` findings to full reporting. `drop` still applies — `--full-scan` widens the *scope*, it does not lower the *bar*.
...
{if any finding has disposition out-of-scope — omit this whole section otherwise:}
...
- **밖으로 미룸**: `out-of-scope` disposition을 가진 finding만. 각 한 줄. 이 섹션의 finding은 `comments` 배열에 넣지 않는다 (Step 6 참조).
...
- Findings with disposition `out-of-scope` (Step 4.5) NEVER appear in the `comments` array either, even when their line maps cleanly. They are body-only, under `밖으로 미룸`. An inline comment is a request for a change in this PR; attaching one to a pre-existing issue asks the author to fix something they did not touch.
